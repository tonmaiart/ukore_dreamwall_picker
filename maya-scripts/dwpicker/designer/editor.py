...
from dwpicker.arrayutils import (
    move_elements_to_array_end,
    move_elements_to_array_begin,
    move_up_array_elements,
    move_down_array_elements,
)
...
class PickerEditor(QtWidgets.QWidget):
    ...
    def __init__(self, document, parent=None):
        ...
        self.shape_canvas = ShapeEditCanvas(self.document, self.display_options)
        ...
        self.shape_library_menu.path_selected.connect(self.create_library_shape)
    ...
    def copy(self):
        clipboard.set([deepcopy(s.options) for s in self.shape_canvas.selection])
    ...
    def invert_selection(self):
        ...
        if self.menu.lock_bg.isChecked():
            shapes = [s for s in self.shape_canvas.selection if not s.is_background()]
    ...
    def option_set(self, option, value):
        ...
        rect_options = ("shape.top", "shape.width", "shape.height", "shape.left")
    ...
    def create_shape(
        self, template, before=False, position=None, targets=None, image=False
    ):
    ...
    def do_symmetry(self, horizontal=True):
        ...
        for shape in shapes:
            if shape.options["shape"] == "custom":
                path_symmetry(path=shape.options["shape.path"], horizontal=horizontal)
                rect_top_left_symmetry(
                    rect=shape.rect,
    ...
    def move_selection(self, direction):
        ...
        self.shape_canvas.transform.shift(self.shape_canvas.selection.shapes, offset)
        for shape in self.shape_canvas.selection:
            shape.synchronize_rect()
            shape.update_path()
        self.shape_canvas.update()
    ...
    def call_context_menu(self, position):
        ...
        method = partial(
            self.create_shape, deepcopy(BUTTON), position=position, targets=targets
        )
        ...
        method = partial(
            self.create_shape, template, position=position, targets=targets
        )
