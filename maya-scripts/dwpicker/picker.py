...
from dwpicker.optionvar import (
    ...
    DEFAULT_HEIGHT,
    DEFAULT_LABEL,
    DISPLAY_HIERARCHY_IN_PICKER,
    LAST_COMMAND_LANGUAGE,
    SYNCHRONYZE_SELECTION,
    ZOOM_SENSITIVITY,
)
...
from dwpicker.shape import build_multiple_shapes, cursor_in_shape, rect_intersects_shape
from dwpicker.stack import create_stack_splitters, count_panels
from dwpicker.selection import (
    select_targets,
    select_shapes_from_selection,
    get_selection_mode,
    NameclashError,
)
...
class PickerStackedView(QtWidgets.QWidget):
    ...
    def create_pickers(self):
        ...
        self.pickers = [
            PickerPanelView(self.document, self.editable, i, self.layers_menu, self)
...
class PickerPanelView(QtWidgets.QWidget):
    ...
    def __init__(self, document, editable=True, panel=0, layers_menu=None, parent=None):
    ...
    def visible_shapes(self):
        return [
            s
            for s in self.document.shapes_by_panel[self.panel]
            if not s.visibility_layer()
    ...
    def reset(self, viewsize=None, selection_only=True):
        shapes = [
            s
            for s in self.visible_shapes()
            if s.options["shape.space"] == "world"
            and not s.options["shape.ignored_by_focus"]
        ]
        shapes_rects = [
            s.bounding_rect() for s in shapes if not selection_only or s.selected
        ]
        if not shapes_rects:
    ...
    def mousePressEvent(self, event):
        ...
        self.interaction_manager.update(
            event, pressed=True, has_shape_hovered=hsh, dragging=bool(self.drag_shapes)
        )
    ...
    def mouseReleaseEvent(self, event):
        ...
        interact = (
            self.clicked_shape
            and self.clicked_shape is hovered_shape
            and self.clicked_shape.is_interactive()
        )
        ...
        if self.interaction_manager.mode == InteractionManager.DRAGGING:
            ...
            self.interaction_manager.mode == InteractionManager.SELECTION
            and not interact
        ):
            try:
        ...
        if not self.clicked_shape:
            ...
            show_context = (
                self.interaction_manager.right_click_pressed
                and not self.clicked_shape.has_right_click_command()
            )
            ...
            if interact:
                button = (
                    "left" if self.interaction_manager.left_click_pressed else "right"
                )
    ...
    def wheelEvent(self, event):
        ...
        if self.zoom_locked:
            return
        factor = 0.25 if event.angleDelta().y() > 0 else -0.25
    ...
    def mouseMoveEvent(self, event):
        world_cursor = self.viewportmapper.to_units_coords(event.pos())
        selection_rect = self.selection_square.rect or QtCore.QRectF(
            world_cursor, world_cursor
        )
    ...
    def call_context_menu(self):
        ...
        shape = detect_hovered_shape(
            self.visible_shapes(), world_cursor, screen_cursor, self.viewportmapper
        )
    ...
    def add_button(self, position, button_type=0):
        ...
        if button_type == 0:
            ...
            text, result = QtWidgets.QInputDialog.getText(self, "Button text", "text")
            if not result:
                return
        ...
        width = max([shape_data["shape.width"], len(shape_data["text.content"]) * 7])
    ...
    def paintEvent(self, _):
        try:
            ...
            shapes = [
                shape
                for shape in self.document.shapes_by_panel[self.panel]
                if not shape.visibility_layer()
                or shape.visibility_layer() not in hidden_layers
            ]
            ...
            if cmds.optionVar(query=DISPLAY_HIERARCHY_IN_PICKER):
                for shape in shapes:
                    ...
                    for child in shape.options["children"]:
                        ...
                        hidden = (
                            child
                            and child.visibility_layer()
                            and child.visibility_layer() in hidden_layers
                        )
            ...
            if self.selection_square.rect:
                draw_selection_square(painter, self.selection_square.rect)
...
class PickerMenu(QtWidgets.QMenu):
    def __init__(self, global_commands=None, shape=None, editable=True, parent=None):
